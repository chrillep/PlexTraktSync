...
class HttpRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.set_response()
        self.wfile.write(
            f"PlexTraktSync Webhook for Tautulli. See {TAUTULLI_WEBHOOK_URL}".encode(
                "utf-8"
            )
        )
...
@click.command()
@click.option(
    "--bind", help="Address to listen on", show_default=True, default="localhost"
)
...
def webhook(bind: str, port: int):
    ...
    with socketserver.TCPServer(
        (bind, port), HttpRequestHandler, bind_and_activate=False
    ) as httpd:
